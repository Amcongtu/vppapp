...
function ProductDetail(props) {
    ...
    const [imageProduct, setImageProduct] = useState(props.data.image[0])
    const imageThumbnail = props.data.image.map((item,index)=>{
        console.log(activeThumnail)
        return(
                ...
                <div className="grid grid-cols-5 gap-6">
                    ...
                    <div className="col-span-3">
                        <div>
                            ...
                            <h2 className='text-[#343434] font-[600] text-[22px] mb-4 mt-1'>{name.id}</h2>
                            <div className='gap-4 flex mb-[12px]'>
                                <span className='block text-[#e54910] text-[26px] font-[600]  leading-[36.75px]'>48.000đ</span>
                            ...
                            <div className='flex mb-5'>
                                ...
                                <div className='gap-2 flex '>
                                    <div className='my-auto'>
                                        <AiFillMinusCircle className='select-none hover:opacity-90 text-main-bg text-[26px] active:scale-95 cursor-pointer' onClick={()=>{if(Number(number)<=1){setNumber(1)}else{setNumber(Number(number)-1)}}}/>
                                    </div>
                                    <input type="number" min={1}  value={number} onChange={e=>setNumber(e.target.value)} className='w-[50px] pl-[12px] text-[14.75px] font-[500] text-center h-[34px] outline-none bg-[#f7f4f4] text-[#343434] rounded-md'/>
                                    <div className='my-auto'>
                                        <AiFillPlusCircle className='select-none hover:opacity-90 text-main-bg text-[26px] active:scale-95 cursor-pointer' onClick={()=>setNumber(Number(number)+1)}/>
                                    </div>
                                </div>
                            </div>
                            <div className='flex gap-5 select-none'>
                                ...
                                <div className='border-[1px] border-[#e54910] bg-[#e54910] hover:opacity-80 duration-150 text-white rounded-md text-[16.75px] px-[20px] py-[12px] font-[500] cursor-pointer active:scale-90'>Thêm vào giỏ hàng</div>
